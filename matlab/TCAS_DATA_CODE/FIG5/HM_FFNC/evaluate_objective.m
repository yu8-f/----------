...
% In H. P. Schwefel and R. M舅ner, editors, Parallel Problem Solving from
...
% ---------------------------------------------------------
% 1. FFNCの伝達関数の定義 (論文 Section III-B)
% ---------------------------------------------------------
% Hsf_s は元のコードの Hlf_ext と同一です（サンプリングフィルタ特性）
Hsf_s = Hlf_ext; 

% ゲイン誤差（mismatch）を定義します。
% 0.0 なら完全キャンセル、0.2 なら +20% の誤差を意味します。
mismatch = 0.0; 

% H_FFNC(s) の計算（論文 式9, 式11の拡張）
Hffnc = (Nhm ./ Ndiv) .* (1 - (1 + mismatch) .* Hsf_s);

% ---------------------------------------------------------
% 2. 外部PLLノイズの伝達計算（FFNC適用）
% ---------------------------------------------------------
% 外部VCOノイズのメイン出力への伝達（論文 式8）
Svco1_out = (abs(Hvco1 .* Hffnc .* Hclose2) .^ 2) .* n_psd_vco1;

% リファレンスノイズのメイン出力への伝達（論文 式12）
% 元コードの Hopen1 は H_OLA(s) に相当します
Href_ffnc = N1 .* (Hopen1 + Hsf_s) ./ (1 + Hopen1) .* (Nhm ./ Ndiv) .* Hclose2;
Sref1_out = (abs(Href_ffnc) .^ 2) .* n_psd_ref;

% 外部PLL由来のノイズを統合
Sext_out = Svco1_out + Sref1_out;

% ---------------------------------------------------------
% 3. メインPLLノイズの伝達と全体の統合
% ---------------------------------------------------------
Svco2 = (abs(Hvco2) .^ 2) .* n_psd_vco2;
Sdsm = (abs(Hclose2) .^ 2) .* n_psd_dsm;

Stotal = Svco2 + Sdsm + Sext_out;
