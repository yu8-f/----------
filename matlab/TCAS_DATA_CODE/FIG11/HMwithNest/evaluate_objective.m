...
%Nested PLL
KpfdcpN =  Icp/(2*pi); % A/rad
Kvco_nest = 2*pi*100e6;
Wc_nest = Fc_nest*(2*pi); % rad/s

%Calculate LPF parameters
R1N = Wc_nest./(KpfdcpN.*Kvco_nest);
C1N = a.*KpfdcpN.*Kvco_nest./(Wc_nest.^2);
C2N = KpfdcpN .* Kvco_nest ./(a.*Wc_nest.^2);
Hlf_nest = (1+s.*R1N.*C1N)./(s.*(C1N+C2N)+R1N.*C1N.*C2N.*s.^2);
 %% Output Frequency
%Loop Filters
...
Hvco1 =  1 ./ (1+Hopen1);
%Nested PLL
HopenN = KpfdcpN .* Hlf_nest .* (Kvco_nest./s);
HcloseN = HopenN./(1+HopenN);
HvcoN = 1./(1+HopenN);



%Main PLL
Hopen2 = Kpfdcp .* Hlf_main .* (Kvco_main ./ s).*(1-(HcloseN.*(1./Npre./Nafter)));
%Hopen2dB = 20 .* log10(abs(Hopen2));

Hclose2 =  Kpfdcp .* Hlf_main .* (Kvco_main ./ s) ./ (1 + Hopen2);
...
%Nested PLL
n_psd_vcoN = 4.*pi.*k.*T.*(1+y) .* (Fext ./ 2 ./ Q ./f) .^2 ./ (P_vco3.*1e-3);
%HM PLL
...
%Nested PLL
SvcoN = (abs(HvcoN) .^ 2) .* n_psd_vcoN;
%Main PLL
Svco2 = (abs(Hvco2) .^ 2) .* n_psd_vco2;
Sdsm = (abs(Hclose2.*HcloseN.*(1/Nafter)) .^ 2) .* n_psd_dsm;
SHM = (abs(Hclose2) .^ 2) .* n_psd_HM;
Sext_out = (abs((Nhm./Ndiv).*Hclose2) .^ 2) .* Sext;
Snest_out = (abs((1./Nafter).*Hclose2) .^ 2) .* SvcoN;
%Sdsm = (abs(Hclose2) .^ 2) .* n_psd_dsm;
%SdsmdB = 10 .* log10(Sdsm);

Stotal = Svco2 + Sdsm + Sext_out + SHM + Snest_out;

A = Stotal .* 10e3;
A_T = sum(A);
J = (sqrt(2 * A_T) ) / (2 * pi * Fout);
obj(1) = (sqrt(2 * A_T) ) / (2 * pi * Fout);
obj(2) = P_vco1+P_vco2+P_vco3+P_PD+P_HM;
