...
CS2 = 1e-12;
Kspd = (N *Wc) / Kvco;
Hlf = (1 ./ (1 + ((CS2 / (CS1 * Fref)) .* s))) .* ((cos((pi .* f) ./ Fref)) - (1j .*sin((pi .* f) ./ Fref))) .* ((Fref .* (sin((pi .* f) ./ Fref))) ./ (pi .* f)); % KL's JSSC'16, Eq.6
Fout = Fref * N; %% Output Frequency
%Loop Filters

%Loop Transfer Functions
%Main PLL
Hopen2 = Kspd .* Hlf .* (Kvco ./ s) .* (1/N);
...
Sref = (abs(Hclose2) .^ 2) .* (n_psd_ref+n_psd_DTC+n_psd_pd);
